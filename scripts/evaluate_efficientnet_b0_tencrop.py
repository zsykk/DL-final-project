...
from evaluate_efficientnet_b0 import (
...
def discover_tencrop_experiments(checkpoint_dir: Path, metrics_dir: Path) -> list[str]:
    return [name for name in discover_experiments(checkpoint_dir, metrics_dir) if uses_tencrop(metrics_dir, name)]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate EfficientNet-B0 TenCrop checkpoints on FER2013 test data.")
...
def main() -> None:
    ...
    names = args.experiment or discover_tencrop_experiments(args.checkpoint_dir, args.metrics_dir)
    if not names:
        raise ValueError("No EfficientNet-B0 TenCrop checkpoints found to evaluate.")

    args.output_dir.mkdir(parents=True, exist_ok=True)
    status(f"Preparing to evaluate {len(names)} EfficientNet-B0 TenCrop checkpoint(s).", "processing")
    ...
    experiment_order = discover_experiments(args.checkpoint_dir, args.metrics_dir)
    dashboard_path = refresh_dashboard(args.output_dir, experiment_order=experiment_order)
